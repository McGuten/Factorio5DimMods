...
local techConfig = {
    ...
    [6] = {
        ...
        prerequisites = { "5d-poison-turret-5", "production-science-pack" }
    },
    [7] = {
    ...
    [8] = {
        ...
        prerequisites = { "5d-poison-turret-7", "utility-science-pack" }
    },
    [9] = {
...
local poisonTurretSpaceAgeMaterials = {
    [6] = { name = "calcite", amount = 10, category = "metallurgy" },
    [7] = { name = "jelly", amount = 12, category = "organic" },
...
local poisonTurretSpaceAgeSciencePacks = {
...
local poisonTurretSpaceAgeDeltaPrerequisites = {
    [6] = "foundry",
    [7] = "bioflux",
...
local function getPoisonTurretIngredients(tier)
    return replaceSharedGunTurretDelta(
        CostCalculator.processIngredients(RecipeTemplates.poisonTurret[tier], tier, {
            skipTierScaling = true,
            skipSpaceAgeMaterials = true
        }),
        tier,
        poisonTurretSpaceAgeMaterials
    )
end

local function getPoisonTurretPrerequisites(tier, basePrerequisites)
    local prerequisites = copyPrerequisites(basePrerequisites)

    if CostConfig.shouldUseSpaceAgeMaterials() then
        addPrerequisiteIfMissing(prerequisites, poisonTurretSpaceAgeDeltaPrerequisites[tier])
    end
...
for tier = 1, 10 do
    ...
    local recipe = {
        ...
        ingredients = getPoisonTurretIngredients(tier),
        results = { { type = "item", name = entityName, amount = 1 } },
        icons = table.deepcopy(itemIcons)
    }
    if CostCalculator.getSpaceAgeRecipeCategory(tier, poisonTurretSpaceAgeMaterials) then recipe.categories = { CostCalculator.getSpaceAgeRecipeCategory(tier, poisonTurretSpaceAgeMaterials) } end
        ...
        tech = {
            ...
            unit = {
                ...
                ingredients = CostCalculator.getTechPacks(tc.basePacks, tier, {
                    spaceAgePackOverrides = poisonTurretSpaceAgeSciencePacks,
                    forceSpaceAgePackOverrides = CostConfig.shouldUseSpaceAgeMaterials()
                }),
                time = 30
            },
            prerequisites = getPoisonTurretPrerequisites(tier, tc.prerequisites)
        }
    end
