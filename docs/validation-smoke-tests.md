...
`scripts/validate-factorio-profiles.ps1` ejecuta validaciones locales de carga para 5Dim sin modificar archivos versionados del workspace. Genera un `mod-list.json`, un `config.ini` y un `write-data` temporales fuera del proyecto para cada perfil y lanza Factorio en modo de validacion de data stage.
